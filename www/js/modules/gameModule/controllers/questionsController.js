...
//    $scope.answeres=$rootScope.gameData.answeresArr;
    ...
    $scope.sendRoundInfo = function () {
        var req = $http.get("http://krockappbackend/index.php?&action=RoundEnd&idgame=" + $scope.currentGame.id + "&score=" + $scope.score + "&ishost=" + $scope.isHost() + "&round=" + $scope.currentRound);
